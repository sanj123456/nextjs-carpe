...
import "./check-eligibility-topup-loan.scss";
...
export default function CheckEligibilityTopUpLoan() {
  ...
  const [finalMinCarEMI, setFinalMinCarEMI] = useState("");
  const [finalMaxCarEMI, setFinalMaxCarEMI] = useState("");
  ...
  const validate = () => {
    ...
    if (!finalMaxCarEMI) {
      toast.error("EMI Tenure is required");
    }
  ...
  }, [fuelType, transmissionType, finalCarYear]);
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      const data = {
        ...
        minMonths: finalMinCarEMI,
        maxMonths: finalMaxCarEMI,
        userName: value.name,
        userMobile: value.phone,
      };
      // console.log(data, "kdklak");
      dispatch(postAllTopupDetails({ data })).then((res) => {
        if (res?.payload?.status == 201) {
          ...
          setErrors({});
          router.push(`/calculator-result?id=${ID}&topup=topup`);
        } else {
          toast?.error(res?.payload?.data?.message);
          setSubmitLoader(false)
        }
  ...
  const ClearedEMI = [
    { minValue: -1, maxValue: -1, placeholder: "None" },
    { minValue: 0, maxValue: 5, placeholder: "0-5 months" },
    { minValue: 6, maxValue: 11, placeholder: "6-11 months" },
    { minValue: 12, maxValue: 17, placeholder: "12-17 months" },
    { minValue: 18, maxValue: 23, placeholder: "18-23 months" },
    { minValue: 24, maxValue: 29, placeholder: "24-29 months" },
    { minValue: 30, maxValue: 35, placeholder: "30-35 months" },
    { minValue: 36, maxValue: 41, placeholder: "36-41 months" },
    { minValue: 42, maxValue: 47, placeholder: "42-47 months" },
    { minValue: 48, maxValue: 53, placeholder: "48-53 months" },
    { minValue: 54, maxValue: 59, placeholder: "54-59 months" },
    { minValue: 60, maxValue: 65, placeholder: "60-65 months" },
    { minValue: 66, maxValue: 71, placeholder: "66-71 months" },
    { minValue: 72, maxValue: 78, placeholder: "72-78 months" },
    { minValue: 79, maxValue: 84, placeholder: "79-84 months" },
  ];
  ...
  const handleEMIClick = (option, option2) => {
    // console.log(option, "optionnnnn");
    setFinalMinCarEMI(option)
    setFinalMaxCarEMI(option2)
    setOpenEmiModal(false)
    setOpenCarValuation(true);
  };
  // console.log(finalMinCarEMI, "finalcaremiii")
  ...
  const handleAllModels = (item) => {
    ...
    if (finalCarBrand && finalCarModel && finalCarYear && finalCarVarient && finalCarMinValue && finalCarMaxValue) {
      if (item == "Ownership") {
        // console.log("I am Kilometer");
        setOpenOwnership(true);
      }
    }
    if (finalCarBrand && finalCarModel && finalCarYear && finalCarVarient && finalCarMinValue && finalCarMaxValue && selectedOwnership) {
      if (item == "ClearedEmi") {
        // console.log("hi I am EMI")
        setOpenEmiModal(true)
      }
    }
  };
  ...
  const handleEligibility = () => {
    ...
    if (data.brand && data.carName) {
      ...
      setOpenKilometersModal(true);
      // setSelectedOwnership("");
    } if (data.brand && data.carName && data.manufacturingYear && data.model && data.maxKms) {
      setOpenSelectModal(false);
      setOpenYearModal(false);
      setOpenVariantModal(false);
      setOpenKilometersModal(false);
      setOpenOwnership(true);
    } if (data.brand && data.carName && data.manufacturingYear && data.model && data.maxKms && data.owner) {
      setOpenSelectModal(false);
      setOpenYearModal(false);
      setOpenVariantModal(false);
      setOpenKilometersModal(false);
      ...
      // setOpenCarValuation(false);
      setOpenEmiModal(true)
    } if (data.brand && data.carName && data.manufacturingYear && data.model && data.maxKms && data.owner && data.maxMonths) {
      setOpenSelectModal(false);
      setOpenYearModal(false);
      setOpenVariantModal(false);
      setOpenKilometersModal(false);
      ...
      {loader ? (
        ...
        <div className="check-eligiblity-wrapper">
          ...
          <div class="caption">
            ...
            <div className="popups-wrapper-topup">
              <div className="popups">
                ...
                <p className="popups-brand-select" onClick={handleBrand}>
                  ...
                  <Image className="polygon" height={6} width={12} alt="dropdown-icon" src="/Polygon 2.svg" />
                </p>
              </div>
              <div className="popups">
              ...
              <div className="popups">
                <p className="popups-brand">Cleared EMI*</p>
                <p
                  className="popups-brand-select"
                  // onClick={() => setOpenOwnership(true)}
                  onClick={() => handleAllModels("ClearedEmi")}
                >
                  {finalMaxCarEMI > 0 ? `${finalMinCarEMI}-${finalMaxCarEMI} months` : <>None</>}{" "}
                  <Image className="polygon" height={6} width={12} alt="dropdown-icon" src="/Polygon 2.svg" />
                </p>
                {/* <p className="popups-brand-select" >Select Owner <img src="/Polygon 2.svg" /></p> */}
              </div>
          ...
          {openEmiModal && (
            <Model setOpenModel={setOpenEmiModal}>
              <div className="kilometer-modal-container">
                <div className="kilometer-modal-header">
                  <img
                    src="/back-arrow.svg"
                    // onClick={() => setOpenKilometersModal(false)}
                    onClick={handleEmiBack}
                  />
                  <h2 className="title">
                    Select the cleared EMI
                  </h2>
                </div>
                <div className="kilometer-modal-body">
                  {ClearedEMI.map((option, index) => (
                    <div
                      key={index}
                      className={`box-emi ${option.maxValue === finalMaxCarEMI ? "active" : ""
                        }`}
                      onClick={() => handleEMIClick(option.minValue, option.maxValue)}
                    >
                      {option.placeholder}
                    </div>
                  ))}
              ...
              <div className="popups-wrapper-topup">
                ...
                <div className="popups">
                  ...
                  <p
                    ...
                    onClick={() => handleAllModels("ClearedEmi")}
                  >
                    {finalMaxCarEMI ? `${finalMinCarEMI}-${finalMaxCarEMI} months` : <>None</>}{" "}
                    <Image className="polygon" height={6} width={12} alt="dropdown-icon" src="/Polygon 2.svg" />
                  </p>
                  {/* <p className="popups-brand-select" >Select Owner <img src="/Polygon 2.svg" /></p> */}
                </div>
